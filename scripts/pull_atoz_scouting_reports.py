...
URLS_PATH = SOURCES_DIR / "manual" / "atoz_scouting_urls_2026.csv"
MANUAL_REPORTS_PATH = SOURCES_DIR / "manual" / "atoz_scouting_reports_2026.csv"
ANALYST_SEED_PATH = SOURCES_DIR / "analyst_rankings_seed.csv"
ANALYST_REPORTS_PATH = PROCESSED_DIR / "analyst_reports_2026.csv"
PROCESSED_PATH = PROCESSED_DIR / "atoz_scouting_structured_2026.csv"
REPORT_PATH = OUTPUTS_DIR / "atoz_scouting_pull_report.md"

POS_RE = r"(?:QB|RB|WR|TE|OT|IOL|EDGE|DT|DL|DE|LB|CB|S|DB|C|G|T)"
TITLE_RE = re.compile(
    rf"(?P<name>.+?)\s+(?P<year>2026)\s+NFL Draft Scouting Report(?:\s+for)?\s+(?P<school>[^|<]+)",
    re.I,
)
...
def _extract_title_identity(page_html: str) -> tuple[str, str, str]:
    ...
    if not m:
        return "", "", ""
    name = _safe_str(m.group("name")).strip("- ")
    school = _safe_str(m.group("school")).split("-")[0].strip()
    pos_m = POS_IN_TITLE_RE.search(title_txt)
    pos = normalize_pos(_safe_str(pos_m.group("pos")).upper()) if pos_m else ""
    return name, pos, school


def parse_atoz_report(page_html: str, source_url: str, snapshot_date: str) -> dict:
    ...
    strengths = _find_section(lines, {"strengths", "pros", "best trait", "what he does well"}, stops)
    concerns = _find_section(lines, {"weaknesses", "concerns", "cons", "areas to improve"}, stops)
    ...
    return {
        ...
        "atoz_rank": source_rank if source_rank is not None else "",
        "atoz_strengths": strengths,
        "atoz_concerns": concerns,
        "atoz_summary": summary,
    }
...
def _load_manual_reports(path: Path, snapshot_date: str) -> list[dict]:
    ...
    with path.open() as f:
        for row in csv.DictReader(f):
            source_rank = _to_int(row.get("source_rank", row.get("atoz_rank", row.get("rank", ""))))
            ...
            payload = {
                ...
                "atoz_rank": source_rank_val,
                "atoz_strengths": _safe_str(row.get("atoz_strengths", row.get("strengths"))),
                "atoz_concerns": _safe_str(row.get("atoz_concerns", row.get("concerns"))),
                "atoz_summary": _safe_str(row.get("atoz_summary", row.get("summary"))),
            }
...
def _merge_analyst_seed(rows: list[dict]) -> int:
    ...
    keep = [r for r in existing if _safe_str(r.get("source")) != ATOZ_SOURCE]
    additions = []
    for r in rows:
...
def _write_report(path: Path, rows: list[dict], attempted: int, failed: int, warnings: list[str], added_seed: int, added_reports: int) -> None:
    ...
    lines = [
        "# A to Z Scouting Reports Pull Report",
        "",
    ...
    for r in top:
        lines.append(
            ...
            f"{_safe_str(r.get('school'))} | {'yes' if _safe_str(r.get('atoz_strengths')) else 'no'} | "
            f"{'yes' if _safe_str(r.get('atoz_concerns')) else 'no'} |"
        )
    path.write_text("\n".join(lines))


def main() -> None:
    p = argparse.ArgumentParser(description="Pull A to Z scouting report rationale into structured scouting features.")
    p.add_argument("--urls-csv", default=str(URLS_PATH), help="CSV file with `source_url`/`url` rows for A to Z reports.")
    p.add_argument(
        "--manual-reports-csv",
    ...
    if not args.skip_fetch:
        ...
        if not url_rows:
            warnings.append("A to Z URLs CSV missing/empty; fetch phase skipped.")
        for row in url_rows:
            ...
            try:
                ...
                parsed = parse_atoz_report(page_html, source_url=url, snapshot_date=snapshot_date)
                ...
                if not parsed.get("source_rank"):
                    ...
                    parsed["atoz_rank"] = parsed["source_rank"]
                if parsed.get("player_name") and parsed.get("position"):
                    rows.append(parsed)
                else:
                    failed += 1
                    warnings.append(f"A to Z parse incomplete for URL: {url}")
            except Exception as exc:
                failed += 1
                warnings.append(f"A to Z fetch failed for {url}: {exc}")
    ...
    if not rows:
        ...
        if manual_rows:
            ...
            warnings.append("A to Z manual reports CSV missing/empty.")
